...
function initLegalModal() {
  ...
  infoBtn.setAttribute("aria-controls", "legal-modal");
  infoBtn.setAttribute("aria-expanded", "false");
  ...
  document.addEventListener("keydown", (event) => {
    if (event.key === "Escape" && !legalModal.hidden) closeModal();
  });
  ...
  if (countrySelect) {
    ...
    countrySelect.addEventListener("change", (event) => {
      ...
      if (fiscalUrls[value]) {
        window.open(fiscalUrls[value], "_blank", "noopener,noreferrer");
      } else if (value === "OTHER") {
